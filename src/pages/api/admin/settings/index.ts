    ...
    try {
      ...
      if (!settings) {
        return res.status(404).json({
          message:
            "Settings not found please check the database incase someone deleted it.",
        });
      }
      const { contactNumber, deliveryCharge, facebook, storeAddress, tiktok } =
        req.body;
      ...
      res
        .status(200)
        .json({ message: "Settings has been updated successfully." });
    } catch (error) {
      console.error(error);
      res.status(500).json({
        error,
