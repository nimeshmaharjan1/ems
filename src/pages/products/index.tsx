...
const Home: NextPageWithLayout = () => {
  ...
  const {
    ...
    ['fetchProducts', shopBySearchParams, currentPage, limit, selectedSearchProduct],
    async () => {
      const params = new URLSearchParams(shopBySearchParams);
      const response = await axios.get(
        `/api/products?${params}&page=${currentPage}&limit=${limit}&title=${selectedSearchProduct?.label || ''}`
      );
  ...
  const loadSearchProducts = async (searchValue: string, loadedData: any, { page }: any) => {
    try {
      ...
      if (!searchValue) {
        ...
        filteredOptions = response.products.filter((option: any) => option.title.toLowerCase().includes(searchValue.toLowerCase()));
      }
      return {
  ...
  return (
    <>
            ...
            formatOptionLabel={(product: any) => {
              return (
                <div className="product-option flex items-center gap-x-3">
                  <Image width={80} height={50} src={product?.image} className="rounded" alt="product-image" />
      ...
      {isError ? (
        ...
        <div className="md:mt-16 text-warning">No products are available at this moment.</div>
      ) : (
        <>
          {' '}
